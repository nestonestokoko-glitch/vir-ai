...
export default function ClipPlayerCanvas({ clip, autoPlay = false }: ClipPlayerCanvasProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div
        ref={containerRef}
        className="relative w-full overflow-hidden rounded-2xl border border-[#1f293d] bg-black shadow-2xl"
        style={{
      ...
      >
        ...
        {clip.videoUrl && !videoError ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-[#0b0f19] via-[#111827] to-[#030712]">
            <div className="h-16 w-16 animate-pulse rounded-full bg-gradient-to-br from-[#0488C5] to-[#526EF5]" />
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <span className="rounded-full bg-emerald-500/20 px-2.5 py-1 text-[11px] font-extrabold text-emerald-400 border border-emerald-500/30 backdrop-blur-md">
            AI Score: {clip.score.overallScore}
          </span>
          <span className="rounded-full bg-slate-900/80 px-2.5 py-1 text-[11px] font-bold text-gray-300 border border-slate-700 backdrop-blur-md">
        ...
        {!isPlaying && (
          ...
          >
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-r from-[#0488C5] to-[#526EF5] text-white shadow-2xl shadow-[#0488C5]/50 transition-transform hover:scale-110">
      ...
      {/* Control Bar */}
      <div className="mt-3 flex w-full items-center justify-between gap-3 rounded-xl border border-[#1f293d] bg-[#0d1322] px-4 py-2.5">
        <div className="flex items-center gap-2">
          <button
            onClick={togglePlay}
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#1e293b] text-white hover:bg-[#0488C5]"
          >
            {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4 fill-current ml-0.5" />}
          </button>
          <button
            onClick={handleRestart}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-[#1e293b] hover:text-white"
          >
        ...
        <input
          ...
          className="h-1.5 flex-1 cursor-pointer appearance-none rounded-full bg-slate-800 accent-[#0488C5]"
        />
